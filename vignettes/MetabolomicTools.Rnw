...
Here, we convert \Robject{sd02\_deconvoluted} into a file in .msp format. 
\Rfunction{convert2MSP} creates an entry for each precursor and 
...
of all fragment ions belonging to the specific precursor.
<<eval=c(1:2)>>=
## identify precursor mz
finalMSP <- convert2MSP(sd02_deconvoluted, split = " _ ", splitInd = 2)

## optional:
## write finalMSP to idMSMStoMSP.msp
...
stored in \Robject{idMSMStissueproject.Rdata} as \Robject{tissue}. Often, 
the data is not in the right format to analyse it, thus we need to bring 
...
<<eval=TRUE>>=
load(system.file("data/convertExampleDF.RData", 
                 package = "MetabolomicTools")) 
@
...
m/z values might differ less than the retention time between \code{sd01} and 
\code{sd02}. Please note, that it might be problematic to compare sd01 and 
sd02 and allocate precursor ions therefrom, especially when data were 
...
\code{tol} for tolerance). \Rfunction{binning} applies the following algorithm: 
...
\begin{itemize}
    ...
    \item from these newly binned together cluster it will calculate the 
        new m/z value and 
    \item calculates the new distance to neighbour fragments.
...
binnedMSP <- binning(msp = finalMSP, tol = 0.01, compartment = compartment)
@
...
to cluster similar features. Therefore we enter:
...
calculate neutral losses from msp files. 
...
## (binning)
mlBinnedMSP <- binning(nlMSP, tol = 0.01, compartment = compartment)
## similarity Matrix
...
\paragraph{Clustering/Visualisation}
Analogously to the .msp file with fragment ions, we are able to cluster the 
similarity matrix based on neutral losses:
<<eval=FALSE>>=
## Clustering
library(amap)
...
<<eval=FALSE>>=
## order newSim according to order of clustering
colnames(nlSimilarityMat)
nlHClustMSP$order
nlNewSim <- matrix(data=NA, nrow = dim(nlSimilarityMat)[1], 
                 ncol = dim(nlSimilarityMat)[1])
nlNewColnames <- colnames(nlSimilarityMat)[nlHClustMSP$order]
colnames(nlNewSim) <- rownames(nlNewSim) <- nlNewColnames
@
...
##dfNameGroup[,"name"] <- paste(
##    sprintf("%04d", 1:length(dfNameGroup[,"name"])), dfNameGroup[,"name"], sep="_")
@
...
dfNameGroupRT <- orderNames(dfNameGroup, similarityMat, order = "clustering",
                                           initialize = TRUE)[[1]]
## create a new similarity matrix with updated rownames
simM <- createOrderedSimMatShiny(dfNameGroupRT, similarityMat)
...
selectedFeatures <- as.character(dfNameGroup[c(1,101,201,301),2])
...
indSelected <- mapply(function(x) which(x == dfNameGroup$name), 
            selectedFeatures)
highlight(dfNameGroup = dfNameGroupRT, ind = indSelected, LinkMatrix = 
            LinkMatrix_threshold)

## plot without highlighting
plotCircos(dfNameGroup, LinkMatrix_cut, initialize = TRUE, featureNames = TRUE, 
    cexFeatureNames = 0.2, groupName = TRUE, links = TRUE, highlight = FALSE, order = "retentionTime")
