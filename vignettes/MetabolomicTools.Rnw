...
use examplary data from \citet{Li2015} to navigate through the 
analysis pipeline. This will include creation of an \code{.msp} object, 
...
\section{Write into .msp file format}
...
\Robject{data.frame} can be accessed by \Robject{msp(finalMSP)}.
<<eval=c(1:2)>>=
...
\subsection{Tissue}
\paragraph{Convert \Robject{idMSMStissueproject.csv} into .msp file format.} 
Here, we would like to analyse data from multiple plant tissues. The data are 
stored in \Robject{idMSMStissueproject.Rdata} as \Robject{tissue}. \newline 
...
finalMSPTissue <- convert2MSP(tissue, split = "_", splitInd=1)
@
...
\Robject{convertExampleDF} by allocating precursor ions to candidate
m/z values based on minimal distance of m/z and deviance of retention time 
...
\begin{itemize}
    ...
    \item the distance will be greater than the tolerande value \code{tol},
...
the length of entries in \Robject{msp}).
...
peak intensity, respectively, of two metabolites. The NDP is calculated 
according to:
\begin{equation}
...
ions together with their respective group. A group can be almost everything and
...
## namesPrec <- rownames(binnedMSPNL) ## for neutral losses
...
\Rfunction{shinyCircos}; this also promotes reproducibility of such figures.
