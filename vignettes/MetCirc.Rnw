...
details) calculation taking into account shared fragments and main neutral 
losses. This vignette 
uses as a case study data from \citet{Li2015} and unpublished MS/MS data 
collected from different organs of tobacco flowers to navigate through the 
analysis pipeline. The pipeline includes creation of an \code{.msp} object, 
...
Here, we convert examplatory data into objects in .msp format that
is used later as input for mass spectral similarity calculations. The .msp
format is an ASCII text format used by mass spectral libraries. The 
...
.msp formats have the following entries: \code{NAME}, \code{RETENTIONTIME},
...
relative intensity in \%. The retention time calcuated by the function 
...
\Robject{data.frame} can be accessed by \Robject{getMSP(mspObject)}.

\subsection{Preparing the sd02\_deconvoluted data set for analysis}
Here, we convert \Robject{sd02\_deconvoluted} into an object in .msp format. 
<<eval=c(1:2)>>=
...
Often, the data is not in the right format to initiate the analysis - so it is 
here - thus we need to bring \Robject{tissue} into the right format to be used as 
input by \Rfunction{convert2MSP}.
...
We would like to restrict the proof-of-funcction analysis to four tissues 
(sepal, SPL; LIM; anther, ANT; 
...
be any biological identifier relevant to the comparitive experiment conducted.
<<eval=TRUE>>=
...
the MSP object with fragments we can bin the \Robject{nlMSP} and create a 
similarity matrix:
<<eval=FALSE>>=
...
selectedFeatures <- as.character(dfNameGroupRT[c(1,101,201,300),2])
