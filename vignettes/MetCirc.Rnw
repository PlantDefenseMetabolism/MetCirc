...
(interactively) explore and annotate metabolomics data. It is especially designed to 
...
The \Rpackage{MetCirc} workflow requires a ata frame
...
"rt" (containing the respective retention time values), "intensity" (intensity
values in \% for the respective fragments). The fourth column requires no 
specific column name, but it has to contain information about the precursor ion
and may additionally contain information about the precursor retention time. 
Alternatively, \Robject{MSP-objects} can also created from msp objects, 
typical data formats for storing MS/MS libraries. Such data frames have to 
contain the row entries "Name:", "Rt=", "Num Peaks:" and information on 
...
time value that is stored in the fourth column. The 
\Robject{data frame} containing information on the number of peaks, fragments
...
The \Robject{split} argument takes a character string specifying how elements
in the fourth column are separated, \Robject{splitIndMZ} and 
\Robject{splitIndRT} take numerical values specifying the position of the 
m/z and retention time value of the precursor ion in the fourth column. 
Furthermore, convert2MSP can take further arguments to retrieve annotation 
...
assignment of compartments to show functionality on how to pass group 
specification. The vector created here can be replaced with 
a vector comprising actual compartments or with any 
...
\code{rt}. The order of these latter columns is not 
...
with S1 and S2 the spectra 1 and 2, respectively, and j the number of columns. 
...
To show and modify annotation enter
<<eval=FALSE>>=
selectedFeatures <- shinyCircos(similarityMat, msp = finalMSP)
@
to the console. This will load information stored in the \Robject{MSP-object}
in the \Robject{shinyCircos} interface. Information (metabolite names and
...
changes to the \Robject{MSP-object}. On exiting \Robbject{shinyCircos} via
...
\Robject{selectedFeatures$x} to retrieve the \Robject{MSP-object}. \newline
